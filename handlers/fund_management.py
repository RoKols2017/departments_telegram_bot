...
from models import Fund, Staff, FundType
...
@with_db_session_async
async def process_birthday_fund_staff(
    message: types.Message, session, state: FSMContext
):
    staff_id = message.text.strip()
    result = await session.execute(
        select(Staff).where(Staff.personnel_number == staff_id)
    )
...
@with_db_session_async
async def process_birthday_fund_deadline(
    message: types.Message, session, state: FSMContext
):
    ...
    new_fund = Fund(
        type=FundType.birthday, deadline=deadline, staff_id=data["staff_id"]
    )
...
async def process_event_deadline(message: types.Message, session, state: FSMContext):
    ...
    data = await state.get_data()
    new_fund = Fund(
        type=FundType.event, deadline=deadline, event_name=data["event_name"]
    )
    session.add(new_fund)
    await session.commit()
